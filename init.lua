...
-- └────────────────────┘
--
-- This is a config designed to mostly use MINI.
-- It provides out of the box a stable, polished, and feature rich Neovim
-- experience.
-- Its structure:
--
-- .
...
-- This will help you better understand your setup.
-- Start with this file.
-- Any order is possible, prefer the one listed above.
-- Ways of navigating your config:
-- - `<Space>` + `e` + (one of) `ikmop` - edit `init.lua` or `plugin/` files.
-- - Inside config directory: `<Space>ff` (picker) or `<Space>ed` (explorer)
-- - Navigate existing buffers with `[b`, `]b`, or `<Space>fb`.
--
-- Config files are also meant to be customized.
-- Initially it is a baseline of
-- a working config based on MINI.
-- Modify it to make it yours.
-- Some approaches:
...
-- - `:h xxx` means "documentation of helptag xxx".
--   Either type text directly followed by Enter or type `<Space>fh` to open a
--   helptag fuzzy picker.
...
-- └────────────────┘
--
-- This config uses `vim.pack` - built-in plugin manager.
-- Its main entry point is a `vim.pack.add()` function, which acts like a
-- "smarter `:packadd`":
-- load plugin after making sure it is installed from source.
-- The state of installed plugins is recorded in the lockfile named
-- `nvim-pack-lock.json`.
-- Example usage:
...
-- Plugin data is passed as argument to the callback.
...
-- - Others are better used only if the above is not enough for good
--   performance.
...
-- The line at the end of this file is called `modeline`.
-- See `:help modeline`.
